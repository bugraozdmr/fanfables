        ...
        <div class="col-md-12 mt-4">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <div class="card-title">Comments Table</div>
                </div>
                ...
                <?php if ($commentCount != 0 && floor($commentCount / $divide) != 0 && !(floor($commentCount / $divide) == 1 && $commentCount % $divide == 0)) : ?>
                    <nav aria-label="Page navigation example" class="mx-2">
                        <ul class="pagination justify-content-end">
                            <?php if ($start > 1) : ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?php echo $dynamicUrl . "/admin/comments.php?cpage=1".(isset($_GET['uquery']) && !empty($_GET['uquery']) ? '&uquery='.$_GET['uquery'] : '').(isset($_GET['cquery']) && !empty($_GET['cquery']) ? '&cquery='.$_GET['cquery'] : '') ?>">1</a>
                                </li>
                                <?php if ($start > 2) : ?>
                                    <li class="page-item">
                            ...
                            <?php for ($i = $start; $i <= $end; $i++) : ?>
                                <li class="page-item">
                                    <a class="page-link <?php echo ($i == $current_page) ? 'active' : '' ?>" style="<?php echo ($i == $current_page) ? 'color:white;' : '' ?>" href="<?php echo $dynamicUrl . "/admin/comments.php?cpage=" . $i.(isset($_GET['uquery']) && !empty($_GET['uquery']) ? '&uquery='.$_GET['uquery'] : '').(isset($_GET['cquery']) && !empty($_GET['cquery']) ? '&cquery='.$_GET['cquery'] : '') ?>"><?php echo $i ?></a>
                                </li>
                            <?php endfor; ?>
                            <!-- Last page -->
                            <?php if ($end < $total_pages) : ?>
                                ...
                                <?php endif; ?>
                                <li class="page-item">
                                    <a class="page-link" href="<?php echo $dynamicUrl . "/admin/comments.php?cpage=" . $total_pages.(isset($_GET['uquery']) && !empty($_GET['uquery']) ? '&uquery='.$_GET['uquery'] : '').(isset($_GET['cquery']) && !empty($_GET['cquery']) ? '&cquery='.$_GET['cquery'] : '')  ?>"><?php echo $total_pages ?></a>
                                </li>
                            <?php endif; ?>
                        </ul>
                    </nav>
...
<script>
    ...
    document.getElementById('delete-button').addEventListener('click', function() {
        ...
        fetch('../actions/show/delete-comment.php', {
                method: 'DELETE',
                headers: {
